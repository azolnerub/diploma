...
import { Shield, RefreshCw, CheckCircle2, Users, Target } from 'lucide-react';
...
export default function Reserve() {
  ...
  const updateFilters = (updates: Record<string, string | null>) => {
    ...
    setSearchParams(params, { replace: true }); // replace: true чтобы не спамить в историю браузера
  };
  ...
  const toggleReserve = async (emp: Employee) => {
    const targetPosId = selectedTargetPositionForEmployee[emp.id];
    const targetRoleId = selectedRoleForEmployee[emp.id];
    if (!emp.in_reserve && !targetPosId) { alert("Выберите целевую должность"); return; }
    try {
      if (emp.in_reserve) await api.delete(`employees/${emp.id}/reserve/remove/`);
      else await api.post(`employees/${emp.id}/reserve/add/`, { position_id: targetPosId, role_id: targetRoleId || null });
      const res = await api.get<Employee[]>('employees/dynamics/');
      setEmployees(res.data || []);
    } catch { alert("Ошибка обновления"); }
  };

  if (authLoading || loading) return <div className="p-20 text-center animate-pulse text-slate-400 font-black text-xs uppercase">Восстановление сессии...</div>;
    ...
    <div className="max-w-7xl mx-auto p-8">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className="text-4xl font-black text-slate-900 tracking-tight">Кадровый резерв</h1><p className="text-slate-500 font-medium mt-1 uppercase text-[10px] tracking-widest">Управление талантами</p></div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredEmployees.map(emp => {
          ...
          let displayRoles = allRoles.filter(r => r.department === null || r.department === emp.department_id);
          if (targetPosId && rolesByPosition[targetPosId]) {
            displayRoles = rolesByPosition[targetPosId];
          }

          return (
            <div key={emp.id} className={`bg-white border rounded-[40px] p-8 transition-all hover:shadow-2xl ${isNeg ? 'border-rose-100 bg-rose-50/20' : 'border-slate-100'}`}>
              <div className="mb-6">
                <h3 className="text-xl font-black text-slate-800 tracking-tight leading-none">{emp.full_name}</h3>
                <p className="text-[10px] font-black uppercase text-slate-400 mt-2 tracking-widest italic">
                   {emp.position_name}
                </p>
              </div>
              ...
              <div className="space-y-6 pt-4 border-t border-slate-50">
                {!emp.in_reserve && (
                  <div className="space-y-2">
                    ...
                    <select 
                      className="w-full px-5 py-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl outline-none font-bold text-xs focus:border-indigo-500 transition-all cursor-pointer"
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    className={`w-full px-5 py-4 border rounded-2xl outline-none font-bold text-xs transition-all appearance-none cursor-pointer ${
                      (targetPosId || emp.in_reserve) ? 'bg-white border-indigo-200' : 'bg-slate-50 border-slate-100 opacity-60'
                    }`}
                    disabled={!targetPosId && !emp.in_reserve}
                    value={selectedRoleForEmployee[emp.id] || ''} 
                    onChange={(e) => setSelectedRoleForEmployee(prev => ({ ...prev, [emp.id]: Number(e.target.value) }))}
                  >
                    <option value="">{ (targetPosId || emp.in_reserve) ? 'Выберите роль...' : 'Сначала выберите должность'}</option>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button 
                    ...
                    className={`flex-1 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 ${
                      emp.in_reserve ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-600 text-white shadow-lg shadow-indigo-100'
                    }`}
                  >
                    {emp.in_reserve ? 'Убрать' : 'В резерв'}
                  </button>
                </div>

                {emp.in_reserve && (
                  <div className="flex justify-center mt-4 animate-in fade-in zoom-in">
                    <span className="text-[9px] font-black uppercase text-emerald-600 bg-emerald-50 px-5 py-2 rounded-full flex items-center gap-2">
                      <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /> Кадровый резерв
                    </span>
                  </div>
                )}
              </div>

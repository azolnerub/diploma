...
export default function RoleBuilder() {
  ...
  // ФИЛЬТР КАТЕГОРИЙ И КОМПЕТЕНЦИЙ
  ...
  // ИТОГОВЫЙ ПРОФИЛЬ (Логика объединения)
  const finalProfile = useMemo(() => {
    ...
    draft.forEach(item => {
      combined[item.competency] = {
        ...
        level: Number(item.required_level) || 0, 
        weight: item.weight, 
        is_key: item.is_key, 
        source: 'role'
      };
  ...
  const availableCompetencies = useMemo(() => {
    ...
    return competencies.filter(c => {
      ...
      const matchesSearch = c.name.toLowerCase().includes(competencySearchTerm.toLowerCase());
      return notUsed && matchesCategory && matchesSearch;
    });
  }, [competencies, draft, selectedPos, posProfiles, selectedCategory, competencySearchTerm]);

  // ИСПРАВЛЕНИЕ: Теперь visibleDraft просто возвращает draft, чтобы ничего не исчезало при вводе
  const visibleDraft = useMemo(() => draft, [draft]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Очистка пустых уровней перед сохранением (превращаем в 0)
      const cleanDraft = draft.map(d => ({
        ...d, 
        required_level: Number(d.required_level) || 0,
        is_key: Boolean(d.is_key)
      }));
      
      await api.post(`roles/${rId}/profile/`, cleanDraft);
  ...
  const startEdit = async (role: Role) => {
    ...
    try {
      ...
      // ИСПРАВЛЕНИЕ: Гарантируем корректное отображение is_key после загрузки
      setDraft(data.map(d => ({
        ...d, 
        is_key: Boolean(d.is_key),
        required_level: d.required_level // Сохраняем как есть
      })));
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div className="relative" ref={editDeptRef}>
                  ...
                  />
                  {selectedDept !== null && (
                    <button onClick={(e) => { e.stopPropagation(); setSelectedDept(null); setEditorDeptSearch(''); }} className="absolute right-10 top-1/2 -translate-y-1/2 text-slate-300 hover:text-rose-500 transition-colors"><X size={14} /></button>
                  )}
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-3 relative z-10">
                {finalProfile.map(item => (
                  <div key={item.id} className={`flex items-center justify-between p-5 rounded-2xl border ${
                    item.is_key ? 'border-rose-500/50 bg-rose-500/10' : // Подсветка ключевой
                    item.source === 'merged' ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-white/5 bg-white/5'
                  }`}>
                    <div className="flex items-center gap-4">
                      <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${
                        item.is_key ? 'bg-rose-500 shadow-lg shadow-rose-500/20' : // Цвет иконки для ключевой
                        item.source === 'merged' || item.source === 'position' ? 'bg-emerald-500 shadow-lg shadow-emerald-500/20' : 'bg-indigo-600'
                      }`}>
                        {item.is_key ? <CheckCircle2 size={20}/> : item.source === 'merged' ? <ArrowUpRight size={20}/> : <Layers size={20}/>}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                           <p className="font-bold text-sm leading-tight">{item.name}</p>
                           {item.is_key && <span className="text-[7px] bg-rose-500 text-white px-1.5 py-0.5 rounded font-black uppercase">Key</span>}
                        </div>
                      ...
                      <div className="flex-1">
                        ...
                        <input 
                          ...
                          onChange={e => {
                            ...
                            if (val === '' || /^\d+$/.test(val)) {
                              setDraft(draft.map(d => d.competency === item.competency ? {...d, required_level: val} : d));
                            }
                          }} 
                          className="w-full bg-white border border-slate-200 rounded-xl py-2 text-center font-black text-xs text-slate-600 focus:ring-2 ring-indigo-500/20" 
                        />
                      </div>
                      <div className="flex-1">
                        ...
                        <input 
                          ...
                          step="0.05" 
                          value={item.weight} 
                          onChange={e => setDraft(draft.map(d => d.competency === item.competency ? {...d, weight: Number(e.target.value)} : d))} 
                          className="w-full bg-white border border-slate-200 rounded-xl py-2 text-center font-black text-xs text-slate-600 focus:ring-2 ring-indigo-500/20" 
                        />
                      </div>
                      <div className="flex-1 flex flex-col justify-end">
                        <button 
                          onClick={() => setDraft(draft.map(d => d.competency === item.competency ? {...d, is_key: !d.is_key} : d))} 
                          className={`w-full py-2 rounded-xl text-[10px] font-black transition-all ${item.is_key ? 'bg-rose-500 text-white shadow-lg shadow-rose-200' : 'bg-slate-200 text-slate-400'}`}
                        >
                          Ключевая
                        </button>
                      </div>
                ...
                <div className="flex flex-col gap-2 mb-4">
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-slate-50 border-none rounded-xl text-xs font-bold"
                        />
                    </div>
                    ...
                    <div className="relative" ref={catRef}>
                        ...
                        {selectedCategory !== null && (
                            <button onClick={(e) => { e.stopPropagation(); setSelectedCategory(null); setCategorySearch(''); }} className="absolute right-10 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500 transition-colors"><X size={14} /></button>
                        )}
                ...
                <div className="space-y-1 max-h-[200px] overflow-y-auto pr-1 custom-scrollbar">
                  {availableCompetencies.map(c => (
                    ...
                    setSelectedCategory(null);
                     ...
                     className="w-full text-left p-3 rounded-xl hover:bg-indigo-50 text-[12px] font-bold text-slate-600 flex justify-between group transition-all">
                      <span className="truncate pr-2">{c.name}</span>
                      <Plus size={14} className="flex-shrink-0 opacity-0 group-hover:opacity-100 text-indigo-600" />
                    </button>
                  ))}
                  {availableCompetencies.length === 0 && (
                      <p className="text-center py-4 text-slate-400 text-[10px] font-bold uppercase">Ничего не найдено</p>
                  )}
                </div>
              </div>
